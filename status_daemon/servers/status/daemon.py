...
class WebSocketServerDaemon(BlockingRunnableWebServer):
    ...
    async def flush_last_statuses(_):
        ...
        Logger.info('Запущена очистка кэша последних статусов...')
        try:
            redis = await RedisController.connect()
            await redis.flush_keys_by_pattern(pattern=RedisController.LAST_PATTERN)
            ...
            Logger.info('Кэш последних статусов успешно очищен.')

    @staticmethod
    async def flush_wish_lists(_):
        """Очищает кэш WISH-листов"""
        Logger.info('Запущена очистка WISH-листов...')
        try:
            redis = await RedisController.connect()
            await redis.flush_keys_by_pattern(pattern=RedisController.WISH_PATTERN)
            await redis.disconnect()
        except Exception as e:
            raise ValueError('Ошибка при попытке очистки кэша WISH-листов: %s' % e)
        else:
            Logger.info('Кэш WISH-листов успешно очищен.')
    ...
    def run(
        ...
        """Запуск"""
        Logger.info('Запущен WebSocket сервер статусов.')
